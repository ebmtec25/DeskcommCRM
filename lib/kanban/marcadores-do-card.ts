 ...
 *
 * O produto tem dois lugares onde se marca, e eles não são o mesmo:
 *
...
/**
 * Os marcadores do card: os do negócio e os do contato, sem repetição.
 *
 * A ordem é estável (negócio primeiro, contato depois) só para a saída ser
 * determinística em teste; quem exibe ordena por conta própria.
 */
export function marcadoresDoCard(lead: Lead): string[] {
  const vistos = new Set<string>();
  const marcadores: string[] = [];
  for (const marcador of [...lead.tags, ...(lead.contact_tags ?? [])]) {
...
/** Este card casa com o marcador filtrado? Pergunta as duas caixas. */
export function cardTemMarcador(lead: Lead, marcador: string): boolean {
  return lead.tags.includes(marcador) || (lead.contact_tags ?? []).includes(marcador);
}
