...
notify pgrst, 'reload schema';
-- ---- conversa arquivada reabre no inbound (migration 0168) ----
-- Arquivar organiza a caixa sem apagar histórico. A regra mora na RPC comum
-- aos canais: inbound reabre; outbound atrasado/eco não desfaz o arquivo.
create or replace function public.fn_mark_conversation_message(
  p_conv uuid, p_direction text, p_preview text, p_at timestamptz
) returns void language plpgsql security definer set search_path = public as $$
begin
...
-- ---- mover leads em lote deixa de empilhar todos na MESMA posição (migration 0178) ----
